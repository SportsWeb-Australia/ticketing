    ...
    <div>
      ...
      {tab === 'report' && !isNew && (
        <div className="space-y-4">
          <Report eventId={eventId!} clubId={clubId} slug={slug} name={name} />
...
// Event-specific gate codes for account-free (PIN) scanning.
// Admins mint and disable; managers can see and copy.
function GateCodes({
  eventId, clubId, role,
}: { eventId: string; clubId: string; role: 'admin' | 'manager' | 'scanner' | null }) {
  const [codes, setCodes] = useState<{ id: string; code: string; label: string; is_active: boolean }[]>([]);
  ...
  const load = async () => {
    const { data } = await supabase.rpc('tk_list_scan_codes', { p_event_id: eventId });
  ...
  const create = async () => {
    setBusy(true); setErr(null);
    const { error } = await supabase.rpc('tk_create_scan_code', {
      p_club_id: clubId, p_event_id: eventId, p_label: label.trim() || 'Gate',
    });
    setBusy(false);
  ...
  const toggle = async (id: string, active: boolean) => {
    await supabase.rpc('tk_set_scan_code_active', { p_code_id: id, p_active: active });
    load();
  };
  ...
  return (
    <Card>
      <h2 className="mb-1 font-semibold text-slate-800">Gate codes</h2>
      <p className="text-xs text-slate-400">
        Lets volunteers scan this event without an account — they open{' '}
        <span className="font-mono">{origin}/scan/code</span> and type the code. Disable it after the event.
      </p>
      ...
      {isAdmin && (
        <div className="mt-3 flex gap-2">
          <input
            ...
            placeholder="Label (e.g. Main gate)" className={inp}
          />
          <button
      ...
      <div className="mt-3 space-y-2">
        {codes.length === 0 && <p className="text-sm text-slate-400">No gate codes yet.</p>}
        {codes.map((c) => (
          <div key={c.id} className="flex items-center justify-between rounded-lg border border-slate-200 px-3 py-2">
            <div className="min-w-0">
              <button
                onClick={() => copy(c.code)}
                title="Tap to copy"
                className="font-mono text-lg tracking-widest text-slate-800"
              >
                {c.code}
              </button>
              <span className="ml-2 text-xs text-slate-400">
                {c.label}{!c.is_active ? ' · disabled' : ''}
              </span>
            </div>
            {isAdmin && (
              <button
